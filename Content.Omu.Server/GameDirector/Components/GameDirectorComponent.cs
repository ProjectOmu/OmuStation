...
namespace Content.Omu.Server.GameDirector.Components;
...
public sealed partial class GameDirectorComponent : Component
{
    ...
    /// <summary>
    ///   How long to wait before reconsidering events when none were run this tick.
    /// </summary>
    [DataField]
    public float NoEventRetryDelay = 30f;

    /// <summary>
    /// All the possible roundstart antags.
    /// </summary>
    // points to the WeightedRandom prototype defined in game_director.yml
    [DataField]
    public ProtoId<WeightedRandomPrototype> RoundStartAntagsWeightTable = "GameDirector";
}

// StoryPrototype and StoryBeatPrototype were moved to Content.Omu.Shared/GameTicking/EventDirector/StoryPrototypes.cs
// so the client can also load them when reading YAMLs. do not redefine them here.
...
public sealed partial class PossibleEvent
{
    ...
    /// </summary>
    // we always set this from code when we build the event list, never from yaml.
    // the = default! part is just to keep the compiler quiet.
    [ViewVariables(VVAccess.ReadWrite)]
    public EntProtoId StationEvent = default!;
    ...
    // empty ctor that nobody really uses, but the yaml loader wants it to exist so we keep it.
