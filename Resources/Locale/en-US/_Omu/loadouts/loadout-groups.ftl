# Nanotrasen Representative
loadout-group-ntr-mask = Nanotrasen Representative mask
loadout-group-ntr-outerclothing = Nanotrasen Representative outer clothing

# Blueshield Officer
loadout-group-bso-belt = Blueshield Officer belt

# Corpsman
loadout-group-corpsman-glasses = Corpsman glasses
loadout-group-corpsman-belt = Corpsman belt
loadout-group-corpsman-headset = Corpsman headset

# Logistics Assistant

loadout-group-logistics-assistant-jumpsuit = Logistics Assistant jumpsuit

# Medical
loadout-group-medical-backpack = Medical backpack

# Paramedic
loadout-group-paramedic-backpack = Paramedic backpack
loadout-group-paramedic-belt = Paramedic belt

# Virologist
loadout-group-virologist-envirohelm = Virologist envirohelm
loadout-group-virologist-envirosuit = Virologist envirosuit
loadout-group-virologist-envirogloves = Virologist envirogloves

# Detective
loadout-group-detective-eyes = Detective Glasses
loadout-group-detective-gun = Detective Gun

# Security
loadout-group-security-gun = Security Gun
loadout-group-sergeant-gun = Sergeant Gun

# Sergeant
loadout-group-sergeant-jumpsuit = Sergeant jumpsuit
loadout-group-sergeant-head = Sergeant head
loadout-group-sergeant-neck = Sergeant neck
loadout-group-sergeant-belt = Sergeant belt

# Bosun
loadout-group-bosun-jumpsuit = Bosun jumpsuit
loadout-group-bosun-head = Bosun head
loadout-group-bosun-neck = Bosun neck
loadout-group-bosun-envirohelm = Bosun envirohelm
loadout-group-bosun-envirosuit = Bosun envirosuit

# Foreman
loadout-group-foreman-jumpsuit = Foreman jumpsuit
loadout-group-foreman-shoes = Foreman Shoes
loadout-group-foreman-head = Foreman head
loadout-group-foreman-neck = Foreman neck
loadout-group-foreman-outerclothing = Foreman outer clothing
loadout-group-foreman-envirohelm = Foreman envirohelm
loadout-group-foreman-envirosuit = Foreman envirosuit

# Medicus
loadout-group-medicus-jumpsuit = Medicus jumpsuit
loadout-group-medicus-head = Medicus head
loadout-group-medicus-outerclothing = Medicus outer clothing
loadout-group-medicus-envirohelm = Medicus envirohelm
loadout-group-medicus-envirosuit = Medicus envirosuit

# Professor
loadout-group-professor-jumpsuit = Professor jumpsuit
loadout-group-professor-head = Professor head
loadout-group-professor-outerclothing = Professor outer clothing
loadout-group-professor-envirohelm = Professor envirohelm
loadout-group-professor-envirosuit = Professor envirosuit

# Clown
loadout-group-clown-belt = Clown belt

# Musician
loadout-group-musician-shoes = Musician Shoes
loadout-group-musician-hat = Musician Hat

# Other
loadout-group-civilian_formal-shoes = Formal shoes
